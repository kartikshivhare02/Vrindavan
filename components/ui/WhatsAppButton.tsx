...
export default function WhatsAppButton() {
  ...
  const message = currentProject
    ? `Hello ${siteConfig.channelPartner.name}, I am interested in ${currentProject.name}. Please share more details.`
  ...
  // Occasional pulse (every 30s, not continuous)
  useEffect(() => {
  ...
  return (
    <a
      ...
      aria-label={`Chat on WhatsApp with ${siteConfig.channelPartner.name}`}
      id="floating-whatsapp-btn"
      title={`WhatsApp ${siteConfig.channelPartner.name}`}
    >
      {/* Pulse ring */}
      {pulsing && (
        <span
